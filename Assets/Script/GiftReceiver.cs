...
public class GiftReceiver : MonoBehaviour
{
    ...
    // 이벤트 콜백들
    public Action<List<GiftData>> OnGiftsLoaded;
    ...
    private void OnGiftBoxLoaded(GetUserDataResult result)
    {
        try
        {
            if (result.Data.ContainsKey("GiftBox") && !string.IsNullOrEmpty(result.Data["GiftBox"].Value))
            {
                ...
                OnGiftsLoaded?.Invoke(currentGifts);
            }
            else
            {
    ...
    {
        // UI 관련 처리는 OnGiftsLoaded 콜백에서 외부에서 처리
    }
    ...
    // 유틸리티 메서드들
    public List<GiftData> GetCurrentGifts()
    {
        return currentGifts;
    }
    public GiftData GetGiftById(string giftId)
    {
        return currentGifts.FirstOrDefault(g => g.giftId == giftId);
    }
